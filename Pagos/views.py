...
from Pagos.models import Pagos, PagosRealizados
...
import Compras.views as compras
...
##########################Pagos realizados##############################
@rol_required('Recepcion')
def nuevoPagoCompras(request, id=None):
    template_name = 'pagos/nuevoPagoCompras.html'
    facturas = FacturaCompra.objects.filter()

    if request.method == "GET":
        pago = PagosRealizados.objects.filter(pk=id).first()
    ...
    if request.method == "POST":
        ...
        if not id:
            pago = PagosRealizados(factura=factura, fecha_pago=fecha_pago, monto_pago=monto_pago, concepto_pago = concepto_pago)
        ...
        if not id:
            messages.error(request, 'No se puede detectar el Nr. de pago')
            return redirect("pagos:listarPagosCompras")

        return redirect("pagos:editarPagoCompras", id=id)

    return render(request, template_name, contexto)

@method_decorator(rol_required('Recepcion'), name='dispatch')
class PagosListViewCompras(generic.ListView):
    model = PagosRealizados
    template_name = 'pagos/listarPagosCompras.html'
    context_object_name = 'pagos'
...
@rol_required('Recepcion')
def eliminar_pagoCompras(request, id_pago):
    if (id_pago != 0):
        req = PagosRealizados.objects.get(id_pago=id_pago)
        req.delete()
        return redirect('pagos:listarPagosCompras')
    else:
        return redirect('pagos:listarPagosCompras')
...
@method_decorator(rol_required('Recepcion'), name='dispatch')
class verFacturasCompras(compras.listarFacturas):
    template_name = "pagos/buscarFacturaCompras.html"